...
autoencoder = tf.keras.models.load_model("hybrid_model.keras")
...
# Compute mean and standard deviation of reconstruction error
...
# Detection rate for Autoencoder
...
iso_forest.fit(X)
iso_forest_scores = iso_forest.decision_function(X)  # Anomaly score
anomalies_iforest = iso_forest.predict(X) == -1

# Detection rate for Isolation Forest
detection_rate_iforest = np.sum(anomalies_iforest) / len(anomalies_iforest)

# Train One-Class SVM
oc_svm = OneClassSVM(nu=0.05, kernel="rbf")
oc_svm.fit(X)
ocsvm_scores = oc_svm.decision_function(X)  # Anomaly score
anomalies_ocsvm = oc_svm.predict(X) == -1

# Detection rate for One-Class SVM
...
# Detection rate for Majority Voting
...
# Add anomaly labels and scores to the dataset
...
df_results["Isolation Forest Score"] = iso_forest_scores
df_results["One-Class SVM Score"] = ocsvm_scores
